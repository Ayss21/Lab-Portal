...
const SignIn = () => {
      ...
      if (window.google && import.meta.env.VITE_APP_GOOGLE_CLIENT_ID) {
        ...
        if (signInDiv) {
          ...
          window.google.accounts.id.renderButton(
            signInDiv,
            { theme: "outline", size: "large", text: "signin_with", width: signInDiv.offsetWidth || 300 }
          );
          console.log("Google Sign-In button rendered.");
        } else {
          console.warn("signInDiv not found for Google button rendering. Ensure the div with id='signInDiv' exists in your JSX.");
        }
      } else {
        console.warn("Google Client ID not found or Google script not ready.");
        setError('Google Sign-In is not configured correctly or failed to load.');
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (activeTab === 'admin') {
      credentialsToSend = { email: formData.email, password: formData.password, adminKey: formData.adminKey };
      loginTypeMessage = "Admin";
    } else { // activeTab === 'user'
      credentialsToSend = { email: formData.email, password: formData.password };
      loginTypeMessage = "User";
    }

    console.log(`Attempting ${loginTypeMessage} sign-in with:`, credentialsToSend.email);
    ...
    try {
      ...
      if (result.success) {
        ...
        const loginMessage = result.user.type === 'admin' ? 'Logged in as Admin!' : 'Sign-in successful!';
        setToastMessage({ type: 'success', message: `${loginMessage} Redirecting...` });
      ...
      console.error(`${loginTypeMessage} Sign-in network/unexpected error:`, err);
  ...
  const handleGoogleCredentialResponse = async (response) => {
    if (activeTab !== 'user') {
      console.warn("Google credential response received while not on user tab. Ignoring.");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setToastMessage({ type: 'success', message: 'Google Sign-in successful! Redirecting...' });
        setLoading(false);
        setTimeout(() => {
          navigate('/');
        }, 1500);
      ...
      setToastMessage({ type: 'error', message: err.message || 'Google Sign-In failed.' });
      setLoading(false);
    }
  };
        ...
        <div>
          ...
          {activeTab === 'admin' && (
            <p className="mt-2 text-center text-sm text-gray-600">
              Staff members, please sign in with your admin credentials. If you don't have an account,{' '}
              <Link
